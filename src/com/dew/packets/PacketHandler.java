...
public final class PacketHandler extends AbstractPacketHandler {
	...
	public void processPacket(Packet packet) {
		...
		case ATTEMPT_LOGIN:
			UserDatabase.loadProfile(
					((CString) packet.getData(0).getObject()).toString(),
					((CString) packet.getData(1).getObject()).toString(),
					packet.getServersideData().serverIndex,
					packet.getServersideData().userID);
			break;
		case MESSAGE:
			System.out.println("Packet was message");
			if (SubServerManager.get(packet.getServersideData().serverIndex)
					.getConnection(packet.getServersideData().userID)
					.isLoggedIn()) {
				packet.getData(0).setObject(packet.getServersideData().username
						+ ": "
						+ ((CString) packet.getData(0).getObject()).toString());
	...
	public boolean sendPacket(Packet p) {
		return SubServerManager.get(p.getServersideData().serverIndex)
				.getConnection(p.getServersideData().userID)
