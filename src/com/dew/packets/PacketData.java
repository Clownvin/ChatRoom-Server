...
public final class PacketData {
	public static PacketData fromBytes(byte[] bytes, int offset) {
		...
		PacketData packetData = new PacketData(
				DataType.getDataType((byte) (bytes[offset] & 0x7F)),
	...
	public byte[] toBytes() throws ArrayLengthException {
		...
		case STRING:
			if (array) {
				...
				if (strings.length > 255) {
					throw new ArrayLengthException(strings.length
							+ " : Array length cannot be greater than 255");
				}
		...
		case INT:
			if (array) {
				...
				int[] ints = (int[]) o;
				if (ints.length > 255) {
					throw new ArrayLengthException(ints.length
							+ " : Array length cannot be greater than 255");
				}
