...
public final class BackupTask implements ThreadTask {
	...
	public void doTask() {
		...
		for (File user : userFolder.listFiles()) {
			if (user != null) {
				if (new Date().getTime() - user.lastModified() < 86400000L
						* daysSinceModToKeep) {
					FileManager.writeBackupFile(
							FileManager.readFile(new UserProfile(0),
									user.getName().replace(".user", "")));
				} else {
					System.out.println("Old file " + user.getName()
							+ ", not backing up and deleting.");
